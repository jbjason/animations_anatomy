...
class DragBottomBody extends StatelessWidget {
  const DragBottomBody(
      {Key? key, required this.isOpen, required this.openDrawer})
      : super(key: key);
  ...
  Widget _appBar(BuildContext context) => Positioned(
        ...
        child: AppBar(
          ...
          leading: isOpen
              ? IconButton(
                  onPressed: () => openDrawer(false, 0.0, 0.0, 1.0),
                  icon: const Icon(Icons.arrow_back_ios, size: 22),
                )
              : IconButton(
                  icon: const Icon(Icons.menu, size: 22),
                  onPressed: () => openDrawer(true, 230.0, 150.0, 0.6),
                ),
        ),
      );
}
